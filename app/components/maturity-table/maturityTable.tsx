...
const MaturityTable: React.FC<MaturityTableProps> = ({
  ...
  const [totalCheckpoint, setTotalCheckpoint] = useState(
    maturityQuestion.length - 1
  );
  ...
  const validateAnswer = () => {
    const currentQuestions = eachLevelQuestion.detail.flatMap(
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formDataMaturity.append(
        "result_answer_maturity",
    ...
    <div className="justify-center items-center max-lg:overflow-x-auto w-full">
      <div className="max-lg:px-4">
        <ProgressBar
          ...
          icons={Array.from({ length: totalCheckpoint + 1 }, (_, i) =>
            String(i + 1)
          )}
      ...
      <Spacer y={8} />
      <p className="font-bold text-xl text-tertiary mb-2">
        {eachLevelQuestion.title}
      </p>
          ...
          <table className="border-collapse border border-gray-400 w-full sm:min-w-full mb-6">
            <thead className="bg-primary top-0 z-10">
              <tr>
                <th
                  colSpan={6}
                  className="border border-amber-700 px-4 py-2 text-secondary"
                >
                  Level {questionSet.level}
                </th>
              </tr>
              <tr>
                <th className="border border-amber-700 px-4 py-2 text-secondary w-[5%]">
                  Kode
                </th>
                <th className="border border-amber-700 px-4 py-2 text-secondary w-[45%]">
                  Pertanyaan
                </th>
                <th className="border border-amber-700 px-4 py-2 text-secondary w-[5%]">
                  Ya
                ...
                <th className="border border-amber-700 px-4 py-2 text-secondary w-[20%]">
                  Evidence
                </th>
                <th className="border border-amber-700 px-4 py-2 text-secondary w-[20%]">
                  Rekomendasi
            ...
            <tbody>
              {questionSet.question?.map((data, qIndex) => {
                const newKode = `${
                  prefixKode[currentCheckpoint]
                }${questionCounter.toString().padStart(2, "0")}`;
                questionCounter += 1;
                return (
                  <tr key={qIndex}>
                    ...
                    <td className="border border-gray-400 px-4 py-2 text-center">
                      <input
                        ...
                        name={`row-${data.id}-ya`}
                        checked={
                          formData.find((item) => item.id_question === data.id)
                            ?.ya || false
                        }
                    ...
                    <td className="border border-gray-400 px-4 py-2 text-center">
                      {formData.find((item) => item.id_question === data.id)
                        ?.ya ? (
                        formData
                          .find((item) => item.id_question === data.id)
                    ...
                    {qIndex === 0 && (
                      <td
                        ...
                        rowSpan={questionSet.question?.length}
                      >
      ...
      <div className="flex justify-end gap-2">
        <Button
          ...
          className={`text-secondary w-24 text-center ${
            currentCheckpoint === 0
              ? "disabled"
              : "hover:bg-red-700 hover:text-white bg-primary"
          }`}
        ...
        <Button
          ...
          className={`text-secondary w-24 text-center ${
            currentCheckpoint === totalCheckpoint
              ? "hover:bg-red-700 hover:text-white bg-primary"
              : "hover:bg-red-700 hover:text-white bg-primary"
          }`}
          onClick={() => {
            if (currentCheckpoint === totalCheckpoint) {
              if (isAdmin) {
                ...
                if (validateAnswer()) {
                  ...
                  window.scrollTo({ top: 0, behavior: "smooth" });
                  toast.error(
                    "Silakan pilih jawaban untuk semua pertanyaan sebelum melanjutkan"
                  );
                }
              }
            } else {
        ...
        >
          <h2
            className={`text-secondary ${
              currentCheckpoint === totalCheckpoint ? "font-bold" : ""
            }`}
          >
            {isAdmin
              ? "Next"
